#!/usr/bin/env python3
"""Presocratics Extraction Script.
...
Diels-Kranz chapter identifiers are properly mapped to specific philosophers
instead of generic chapter numbers.
...
class PresocraticsExtractor(_generic.UnifiedExtractor):
    """Extractor with Diels-Kranz philosopher name mapping."""

    def __init__(self) -> None:
        super().__init__()

        # Load chapter → philosopher name mapping from CSV (if available)
        self.chapter_philosopher_map: Dict[str, str] = {}
        csv_path = (
            Path(__file__).parent.parent
            / "data"
            / "input"
            / "presocratic_name_hyperlinks.csv"
        )
        if csv_path.exists():
            try:
                with open(csv_path, "r", encoding="utf-8-sig") as fh:  # Handle BOM
                    ...
                    for row in reader:
                        ...
                        if text and philosopher:
                            self.chapter_philosopher_map[text] = philosopher
    ...
    def _sort_key(cls, ref: str) -> tuple[int, str, int]:
        """Return a tuple that sorts references naturally (1-A,1 < 1-A,2 < 1-B,1)."""
        
        try:
            # Parse reference like "22-B,30" -> chapter=22, type="B", num=30
            if "," in ref:
                chapter_part, num_part = ref.split(",", 1)
                num = int(num_part.rstrip("abc"))  # Handle sub-fragments like "30a"
            else:
                chapter_part = ref
                num = 0
            
            if "-" in chapter_part:
                chapter_str, type_str = chapter_part.split("-", 1)
                chapter = int(chapter_str)
                return (chapter, type_str, num)
            else:
                # Fallback for unexpected format
                return (999, chapter_part, num)
    ...
    async def extract_fragment(self, fragment: Dict[str, Any]) -> Optional[ExtractedText]:
        """Extract a single Presocratics fragment with proper philosopher mapping."""
        ...
        # Derive philosopher mapping from reference
        # For references like "22-B,30", we want to map "22-B" to philosopher
        chapter_section = reference.split(",")[0] if "," in reference else reference
        
        philosopher: str = self.chapter_philosopher_map.get(
            chapter_section,
            fragment.get("philosopher", f"Chapter {chapter_section}")
        )
...
async def _main() -> None:  # noqa: D401
    ...
    parser = argparse.ArgumentParser(description="Presocratics Daphnet Extraction")
    parser.add_argument(
        "--catalogue",
        default=str(Path(__file__).parent.parent / "data" / "catalogues" / "presocratics_catalogue.json"),
        help="Path to the Presocratics catalogue JSON (auto-detected by default).",
    )
    parser.add_argument("--no-resume", action="store_true", help="Ignore checkpoint and start fresh.")

    args = parser.parse_args()

    extractor = PresocraticsExtractor()
    # Run using fixed collection name 'presocratics'
    await extractor.run(collection="presocratics", resume=not args.no_resume)
